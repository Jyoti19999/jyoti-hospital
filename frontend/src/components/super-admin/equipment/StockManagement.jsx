// src/components/super-admin/equipment/StockManagement.jsx
...
const StockManagement = () => {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      const response = await equipmentService.getStockTransactions(selectedEquipment.id, {
        limit: 20
      });
  ...
  const handleStockOperation = async () => {
    if (!selectedEquipment || !stockForm.quantity || !stockForm.reason) {
      toast.error('Please fill in all required fields');
      return;
    }

    // Additional validation for adjust operation
    if (stockOperation === 'adjust' && stockForm.reason.length < 5) {
      toast.error('Reason must be at least 5 characters for stock adjustment');
      return;
    }
    ...
    try {
      ...
      switch (stockOperation) {
        case 'add':
          response = await equipmentService.addStock(selectedEquipment.id, stockData);
          break;
        case 'remove':
          response = await equipmentService.removeStock(selectedEquipment.id, stockData);
          break;
        case 'adjust':
          response = await equipmentService.adjustStock(selectedEquipment.id, {
      ...
      if (response.success) {
        ...
        // Update selected equipment data
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Manage equipment stock levels, add/remove inventory, and track stock movements
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {/* Selected Equipment Details & Actions */}
                ...
                <>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-blue-600">
                            {selectedEquipment.currentStock}
                          </div>
                          <div className="text-xs text-muted-foreground">Current Stock</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-semibold text-orange-600">
                            {selectedEquipment.reorderLevel}
                          </div>
                          <div className="text-xs text-muted-foreground">Reorder Level</div>
                        </div>
                        <div className="text-center">
                          <Badge variant={getStockStatus(selectedEquipment.currentStock, selectedEquipment.reorderLevel).variant}>
                            {getStockStatus(selectedEquipment.currentStock, selectedEquipment.reorderLevel).label}
                          </Badge>
                          <div className="text-xs text-muted-foreground mt-1">Status</div>
                        </div>
                        <div className="text-center">
                          <div className="text-lg font-semibold text-green-600">
                            ₹{selectedEquipment.unitCost || 0}
                          </div>
                      ...
                      <div className="flex gap-2 mt-4">
                        <Dialog open={showStockDialog} onOpenChange={setShowStockDialog}>
                          ...
                          <DialogContent>
                            <DialogHeader>
                              <DialogTitle>
                                {stockOperation === 'add' && 'Add Stock'}
                                {stockOperation === 'remove' && 'Remove Stock'}
                                {stockOperation === 'adjust' && 'Adjust Stock'}
                              </DialogTitle>
                              <DialogDescription>
                                {stockOperation === 'add' && 'Add new stock to increase inventory levels'}
                                {stockOperation === 'remove' && 'Remove stock for consumption or disposal'}
                                {stockOperation === 'adjust' && 'Manually adjust stock to correct inventory'}
                              </DialogDescription>
                            </DialogHeader>
                  ...
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        Stock movement history for this equipment
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <p className="text-xs text-muted-foreground">
                                  {formatDate(transaction.transactionDate)} • {transaction.performedByName || 'Unknown Staff'}
                                  </p>
                                </div>
                              </div>
                ...
                <Card>
                  <CardContent className="text-center py-12">
                    ...
                    <p className="text-muted-foreground">
                      Choose an equipment item from the left to manage its stock
                    </p>
                  </CardContent>
                </Card>
              )}
